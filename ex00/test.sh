...
test_should_guarantee_correct_name_on_test6_file() {
  create_test_files
  if [ $? -eq 1 ]; then
    return 1;
  fi
  cd test_files
  echo $(get_nth_line "$(ls -l)" 8) | read permission hl o g bytes month day hour name
  cd ..

  expected_name="test6 -> test0"
...
test_should_guarantee_correct_permission_on_test6() {
  create_test_files
  if [ $? -eq 1 ]; then
    return 1;
  fi
  cd test_files
  echo $(get_nth_line "$(ls -l)" 8) | read permission _
  cd ..

  expected_permission="l"

  assert_str_eq $expected_permission ${permission:0:1}

  remove_test_files
}

test_should_guarantee_correct_bytes_on_test6_file() {
  create_test_files
  if [ $? -eq 1 ]; then
    return 1;
  fi
  cd test_files
  echo $(get_nth_line "$(ls -l)" 8) | read permission hl o g bytes _
  cd ..
...
test_should_guarantee_correct_timestamp_on_test6() {
  create_test_files
  if [ $? -eq 1 ]; then
    return 1;
  fi
  cd test_files
  echo $(get_nth_line "$(ls -l)" 8) | read permission hard_links owner group bytes month day hour name
  cd ..
